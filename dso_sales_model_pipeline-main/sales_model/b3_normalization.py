...
input_file = 'sales_model/data/3_flooring_capping/train_sample_post_outlier_trtmnt.csv'
output_file = 'sales_model/data/7_post_transformation/train_sample.csv'

def normalize(df, sample):
    df_vars = pd.read_csv('sales_model/data/5_variable_shortlisting/variable_input_file.csv')
    ...
    if isFileValid: 
        ...
        if sample == 'TRAIN':
            ...
            dump(min_max_scaler, open('sales_model/data/4_normalization/train_sample_min_max_scaler.pkl', 'wb'))
            logger.debug("Saved the Training Sample's Normalization Object")
        elif sample == 'TEST':
            min_max_scaler = load(open('sales_model/data/4_normalization/train_sample_min_max_scaler.pkl', 'rb'))
        elif sample == 'BUILD':
            min_max_scaler = MinMaxScaler()
            min_max_scaler.fit(x)
            dump(min_max_scaler, open('sales_model/data/4_normalization/build_sample_min_max_scaler.pkl', 'wb'))
            logger.debug("Saved the Build Sample's Normalization Object")
        elif sample == 'OUTSAMPLE':
            min_max_scaler = load(open('sales_model/data/4_normalization/build_sample_min_max_scaler.pkl', 'rb'))
...
def build_variable_file(df):
    ...
    col_ind = 0
    none_type = ['invoice_yyyymm', 'next_3m_sales', 'next_6m_sales', 'balance_2m', 'balance_3m', 'avg_invoice_amount_2m',
    ...
    df_vars.to_csv('sales_model/data/5_variable_shortlisting/variable_input_file.csv')
