...
# Input Files
input_file = 'sales_model/data/2_combined/train_sample.csv'

# Output Files
output_file = 'sales_model/data/3_flooring_capping/train_sample_post_outlier_trtmnt.csv'
stats_file = 'sales_model/data/3_flooring_capping/variable_stats.csv'
stats_after = 'sales_model/data/3_flooring_capping/stats_after_floor_cap.csv'
full_data_stats_file = 'sales_model/data/3_flooring_capping/build_sample_variable_stats.csv'
...
def floor_cap(df_wo_flooring_capping, sample):
    ...
    sales_var = ['sales', 'sales_till_now', 'sum_invoices_3m_obl', 'sum_invoices_6m_obl', 'sales_3m', 'sales_2m', 'sales_6m', 
                ...
                'avg_invoice_amount', 'avg_invoice_amount_per_month','balance_1', 'balance_2', 'balance_3']
    ...
    if sample == 'TRAIN':
        ...
        df_stats.to_csv('sales_model/data/3_flooring_capping/train_data_variable_stats.csv')
        logger.debug("Stats File Saved for Training Sample Saved")
    elif sample == 'TEST':
        df_stats = pd.read_csv('sales_model/data/3_flooring_capping/train_data_variable_stats.csv', index_col=0)
    elif sample == 'BUILD':
        df_stats = df_wo_flooring_capping.describe(percentiles=[0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.90, 0.95, 0.99], include='all')
        df_stats.to_csv('sales_model/data/3_flooring_capping/build_sample_variable_stats.csv')
        logger.debug("Stats File Saved for Build Sample Saved")
    elif sample == 'OUTSAMPLE':
        df_stats = pd.read_csv('sales_model/data/3_flooring_capping/build_sample_variable_stats.csv', index_col=0)

    df_w_flooring_capping = df_wo_flooring_capping.copy()

    for column in col_list:
        if column in sales_var:
            row_index = 0
    ...
    if sample == 'TRAIN':
        ...
        after_flooring = 'sales_model/data/3_flooring_capping/post_flooring_capping_build_sample_variable_stats.csv'
