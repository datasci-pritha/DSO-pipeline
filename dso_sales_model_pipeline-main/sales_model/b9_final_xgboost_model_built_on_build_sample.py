...
from sales_model.b2_flooring_capping import floor_cap
from sales_model.b3_normalization import normalize
from config_connection_varirables import conn, session, logger, save_intermediate_files

# Input Data Files
input_file = 'sales_model/data/2_combined/build_sample.csv'
outsample_file = 'sales_model/data/2_combined/outsample.csv'
input_var_file = 'sales_model/data/7_post_transformation/variable_output_file.csv'

# Intermediary Output Files
build_sample_post_transformations = "sales_model/data/9_final_model/post_transformation_whole_data.csv"
out_sample_post_transformations = "sales_model/data/9_final_model/post_transformation_outsample_data.csv"

# Output Data Files
output_build_sample = 'sales_model/data/9_final_model/build_sample_scored_output.csv'
output_out_sample = 'sales_model/data/9_final_model/out_sample_scored_output.csv'
output_all_sample = 'sales_model/data/9_final_model/scored_output.csv'
feature_importance_file = 'sales_model/model_dumps/final_dump/xgboost_feature_importance.csv'
error_metrics_file = 'sales_model/model_dumps/error_metrics.csv'

# Output Plot Files
scatter_plot = 'sales_model/plots/4_final_model_validation/final_scatter_plot_next_2m_sales_actual_vs_next_2m_sales_pred.png'
hist_plot = 'sales_model/plots/4_final_model_validation/final_histogram_next_2m_sales_actual_next_2m_sales_pred.png'
residual_plot = 'sales_model/plots/4_final_model_validation/final_residual_plot.png'
qq_plot = 'sales_model/plots/4_final_model_validation/final_qq_plot.png'
...
def preprocess_all_data(df, df_out):
    # Floor the Build Sample & Out Sample
    df_floor = floor_cap(df, 'BUILD')
    logger.debug("\n\nFlooring and Capping Done on BUILD Sample")
    df_out_floor = floor_cap(df_out, 'OUTSAMPLE')
    ...
    # Preprocessing Data
    df_null_treated = preprocess_data(df_normalize, 'BUILD')
    df_out_null_treated = preprocess_data(df_out_normalize, 'OOS')

    return df_null_treated, df_out_null_treated
...
def preprocess_data(df_data, sample):
    ...
    df_data_processed = df_data.drop(target, axis=1)
...
def train_model(X, y):
    ...
    # Compute VIF
    vif = compute_vif(X, X.columns.to_list())
    ...
    # Save the Model
    dump(xgb_reg, open('sales_model/model_dumps/final_dump/xgboost.pkl', 'wb'))
...
def floor_cap_target_n_save_files(df, df_out, features_we_need, X, y, y_pred, X_out, y_out, y_out_pred):
    ...
    df_pred['next_2m_sales_actual'] = np.where(df_pred['next_2m_sales_actual'] < 1000, 1000, df_pred['next_2m_sales_actual'])
    df_pred['next_2m_sales_actual'] = np.where(df_pred['next_2m_sales_actual'] > 1.71*1e7, 1.71*1e7, df_pred['next_2m_sales_actual'])
    df_pred['next_2m_sales_pred'] = np.where(df_pred['next_2m_sales_pred'] < 1000, 1000, df_pred['next_2m_sales_pred'])
    df_pred['next_2m_sales_pred'] = np.where(df_pred['next_2m_sales_pred'] > 1.71*1e7, 1.71*1e7, df_pred['next_2m_sales_pred'])
    ...
    df_out_pred['next_2m_sales_actual'] = 0
    df_out_pred['next_2m_sales_pred'] = np.where(df_out_pred['next_2m_sales_pred'] < 1000, 1000, df_out_pred['next_2m_sales_pred'])
    df_out_pred['next_2m_sales_pred'] = np.where(df_out_pred['next_2m_sales_pred'] > 1.71*1e7, 1.71*1e7, df_out_pred['next_2m_sales_pred'])
...
def create_all_plots(df_pred):
    ...
    plt.clf()
    plt.scatter(df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_pred'], c='orange')
    plt.plot(df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_actual'], c='black')
    ...
    plt.hist([df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_pred']], bins, label=['actual', 'Model'])
    plt.legend(loc='upper right')
    ...
    plt.clf()
    res = df_pred['next_2m_sales_actual'] - df_pred['next_2m_sales_pred']
...
def get_error_metrics(df_pred, df_error):
    # RMSE
    rmse_build = mse(df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_pred'], squared = False)
    print("RMSE of Build Sample: ", rmse_build)
    logger.debug("\n\n\n RMSE of Training Sample: %s \n\n\n ", rmse_build)

    # R Squared
    r_sq_build = get_r_squared(df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_pred'])
    print("R Squared of Build Sample: ", r_sq_build)
    logger.debug("\n\n\n R Squared of Build Sample: %s \n\n\n ", r_sq_build)

    # MAE
    mae_build = mean_absolute_error(df_pred['next_2m_sales_actual'], df_pred['next_2m_sales_pred'])
...
def build_model(df, df_out, features_we_need, X, y, X_out, y_out):
    # Train the Model
...
if __name__ == "__main__":
    ...
    build_model(df, df_out, features_we_need, X, y, X_out, y_out)
